<?php 
class Clientes extends My_Db_Table_Abstract
{
	protected $_name = 'clientes';
	
	protected $_referenceMap=array(
		'Cidade' => array(
			'columns'	=>	'cidadeID',
			'refTableClass'	=> 'Cidades'
		),
		'Divisao' => array(
			'columns'	=>	'divisaoID',
			'refTableClass'	=> 'Divisoes'
		)
	);

    public $selectBoxes = array(
            'varName'=>'clientes',
            'id'=>'id',
            'displayField' => 'nome',
            'where'=>'ativo = 1',
            'firstOption'=> "blank"
     );

	public $applyFilters = array(
				'all' => array('StringTrim','StringToUpper','HtmlEntities'),
				'limite' => array(
					'LocalizedToNormalized' => array('precision' => 2)
				)
			);

	protected $_validate = array(
			'id'=>array(
				'NotEmpty'	=> array(
					'options'=>array(),
					'message' =>'Id empty?.'
				),
				'Int'	=> array(
					'options'=>array(),
					'message' =>'Id not int?'
				),
				'StringLength' => array(
					'options'=>array(0, 10),
					'message'=>'Id deve ter no máximo 10 caracteres.'
				)
			),
			'cidadeID'=>array(
				'NotEmpty'	=> array(
					'options'=>array(),
					'message' => "O campo 'Cidade' deve ser preenchido."
				),
				'Int'	=> array(
					'options'=>array(),
					'message' => 'Cidade deve ser Inteiro.'
				),
				'Db_RecordExists' => array(
					'options'=>array('cidades', 'id'),
					'message'=> 'Cidade não existe.'
				)
			),
			'divisaoID'=>array(
				'NotEmpty'	=> array(
					'options'=>array(),
					'message' =>'O campo \'Divisão\' deve ser preenchido.'
				),
				'Db_RecordExists' => array(
					'options'=>array('divisoes', 'id'),
					'message'=>'Cidade não existe.'
				)
			),
			'nome'=>array(
				'NotEmpty'	=> array(
					'options'=>array(),
					'message' =>'Fantasia é campo obrigatório.'
				),
				'Db_NoRecordExists'	=> array(
					'options'=>array('clientes','nome'),
					'message' =>"'Nome' já existe."
				),
				'StringLength' => array(
					'options'=>array(0, 45),
					'message'=>"'Fantasia' deve até 45 caracteres."
				),
			),
			'nome_oficial'=>array(
				'Db_NoRecordExists'	=> array(
					'options'=>array('clientes','nome_oficial'),
					'message' =>'Nome já existe.'
				),
				'StringLength' => array(
					'options'=>array(0, 45),
					'message'=>"'Nome' deve ter até 45 caracteres."
				),
			),
			'cpfCnpj'=>array(
				'CPFOrCNPJ'	=> array(
					'options'=>array(),
					'message' =>'CNPJ/CNPJ inválido.',
					'namespace' =>array('My_Validate')
				),
				'Db_NoRecordExists'	=> array(
					'options'=>array('clientes','cpfCnpj'),
					'message' =>'CPF/CNPJ foi cadastrado em um outro registro.'
				),
				'NotEmpty'	=> array(
					'message' =>'O campo CNPJ é obrigatório.'
				)
			),
			'contato'=>array(
				'StringLength' => array(
					'options'=>array(0, 45),
					'message'=>"'Contato' até 45 caracteres."
				),
			),
			'endereco'=>array(
				'NotEmpty'	=> array( 'message' =>"O campo 'Endereço' é obrigatório."),
				'StringLength' => array(
					'options'=>array(0, 80),
					'message'=>"'Endereco' até 80 caracteres."
				),
			),
			'numero'=>array(
				'NotEmpty'	=> array( 'message' =>"O campo 'Numero' é obrigatório."),
				'StringLength' => array(
					'options'=>array(0, 10),
					'message'=>"'Numero' até 10 caracteres."
				),
			),
			'bairro'=>array(
				'NotEmpty'	=> array( 'message' =>"O campo 'Bairro' é obrigatório."),
				'StringLength' => array(
					'options'=>array(0, 45),
					'message'=>"'Bairro' até 45 caracteres."
				),
			),
			'telefone'=>array(
				'NotEmpty'	=> array( 'message' =>"O campo 'Telefone' é obrigatório."),
			),
			'cep'=>array(
				'NotEmpty'	=> array( 'message' =>"O campo 'Cep' é obrigatório."),
				'StringLength' => array(
					'options'=>array(0, 10),
					'message'=>"'Cep' até 10 caracteres."
				),
			),
			'complemento'=>array(
				'StringLength' => array(
					'options'=>array(0, 10),
					'message'=>"'Complemento' até 10 caracteres."
				),
			),
			'inscricaoEstadual'=>array(
				'StringLength' => array(
					'options'=>array(0, 18),
					'message'=>"'Inscrição Estadual' até 18 caracteres."
				),
			),
			'email'=>array(
				'NotEmpty'	=> array( 'message' =>"O campo 'Inscrição estadual' é obrigatório."),
				'EmailAddress'	=> array( 'message' =>"Email inválido."),
				'StringLength' => array(
					'options'=>array(0, 80),
					'message'=>"'Email' até 80 caracteres."
				),
			),
	);




	public function getTotalEmPedidos($id = null)
	{
		if(null === $id)
			$id = $this->id;

		$db = $this->getAdapter();
		$sql = $db->select()
					->from(array('p'=>'pedidos'),array())
					->join(array('i'=>'itemPedido'), 'p.id = i.pedidoID',
						array('total'=> 'SUM(precoCliente*quantidade)')
					)
					->join(array('itq'=>'item_tamanho_quantidade'), 'i.id = itq.item_pedido_id',
						array()
					)
					->where('p.statusID = 1')
					->where('p.clienteID = ?', $id)
					->group('p.id');

		$row = $db->fetchRow($sql);
		return $row['total'];
	}

	public function getSaldoDisponivel(Zend_Db_Table_Row $cliente)
	{
		return ($cliente->limite - $this->getTotalEmPedidos($cliente->id));
	}
}